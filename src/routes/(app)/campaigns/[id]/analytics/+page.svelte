<script lang="ts">
	...
	type FunnelDailyPoint = PageData['funnelDaily'][number];
	...
	const trendData = $derived.by(() =>
		data.funnelDaily.map((point: FunnelDailyPoint) => ({
	...
	const inboundMessagesTotal = $derived.by(() =>
		data.funnelDaily.reduce(
			(total: number, point: FunnelDailyPoint) => total + point.inboundMessages,
			0
		)
	);

	const campaignBarItems = $derived.by(() =>
		data.campaignSummary.slice(0, 6).map((row: CampaignSummaryRow, index: number) => ({
	...
	const sourceBarItems = $derived.by(() =>
		data.sourceMediumPerformance.slice(0, 6).map((row: SourceMediumRow, index: number) => ({
			id: `source-${index}`,
	...
	const campaignRows = $derived.by(() =>
		data.campaignSummary.map((row: CampaignSummaryRow, index: number) => ({
	...
	const sourceRows = $derived.by(() =>
		data.sourceMediumPerformance.map((row: SourceMediumRow, index: number) => ({
	...
	const ctaRows = $derived.by(() =>
		data.ctaPerformance.map((row: CtaRow, index: number) => ({
...
<svelte:head>
	<title>Campaign Studio • {data.campaign.name} Analytics</title>
</svelte:head>

<section class="analytics-page">
	<header class="hero">
		<div>
			<p class="eyebrow">Campaigns › {data.campaign.name} › Analytics</p>
			<h1>Campaign dashboard<span class="dot">.</span></h1>
			<p class="intro">Internal conversion and attribution snapshot filtered to this campaign.</p>
		</div>
	...
	<SectionPanel
		title="Funnel"
		description="Trend and selected-range funnel summary"
		scopeLabel={`${data.dateRange.from} to ${data.dateRange.to}`}
	>
	...
	<SectionPanel
		...
		description="This campaign's conversion summary"
		scopeLabel="All-time rollup"
	>
		<div class="section-grid">
			<BarList title="First-touch bookings" items={campaignBarItems} />
			<DataTable
				columns={[
					'Campaign',
					'Visits',
	...
	<SectionPanel
		title="Source / medium"
		description="Performance by source-medium for this campaign"
		scopeLabel="All-time rollup"
	>
		<div class="section-grid">
	...
	<SectionPanel
		...
		description="CTA comparison for this campaign"
		scopeLabel="All-time rollup"
	>
		<DataTable
			columns={[
