...
const runtimeSource = String.raw`(() => {
  ...
  if (context.runtimeVersion !== 'v2') return;
  ...
  document.querySelectorAll('[data-cs-widget]').forEach(async (node) => {
    ...
    if (widget === 'youtube-video') {
      ...
      const title = (node.getAttribute('data-cs-video-title') || 'YouTube video').trim().slice(0, 120) || 'YouTube video';
      ...
      button.addEventListener('click', () => {
        ...
        iframe.title = title; iframe.loading = 'lazy'; iframe.src = 'https://www.youtube-nocookie.com/embed/' + videoId + '?rel=0';
