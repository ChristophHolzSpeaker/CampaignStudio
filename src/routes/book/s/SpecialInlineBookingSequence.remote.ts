...
import { getSpecialBookingSlotProfile } from './special-booking-profile';
...
export const submitSpecialInlineBooking = form('unchecked', async (rawData) => {
	...
	const currentSlotPreview = await resolvePublicBookingSlotPreview({
		bookingType: 'general',
		slotProfile
	});
	...
	if (
		!isSlotInPublicBookingGroups({
			slotGroups: currentSlotPreview.slotGroups,
