...
const inlineEditInputSchema = z.object({
	...
	sectionType: z.literal('frictionless_funnel_booking'),
...
export const saveFrictionlessFunnelField = command(inlineEditInputSchema, async (input) => {
	...
	if (
		section.type !== 'frictionless_funnel_booking' ||
		input.sectionType !== 'frictionless_funnel_booking'
	) {
		throw new Error('Only Frictionless Funnel fields can be edited here');
	}
	...
	const updatedPage = {
		...
		sections: page.sections.map((item, index) => {
			if (index !== input.sectionIndex || item.type !== 'frictionless_funnel_booking') {
				return item;
			}
