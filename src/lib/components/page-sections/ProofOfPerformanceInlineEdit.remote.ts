...
const inlineEditInputSchema = z.object({
	...
	sectionType: z.literal('proof_of_performance'),
...
export const saveProofOfPerformanceField = command(inlineEditInputSchema, async (input) => {
	...
	if (section.type !== 'proof_of_performance' || input.sectionType !== 'proof_of_performance') {
		throw new Error('Only Proof of Performance fields can be edited here');
	}

	if (typeof section.props.title !== 'string') {
		throw new Error('Field is not editable');
	}

	const currentValue = section.props.title;
	...
	const updatedPage = {
		...
		sections: page.sections.map((item, index) => {
			if (index !== input.sectionIndex || item.type !== 'proof_of_performance') {
				return item;
			}
