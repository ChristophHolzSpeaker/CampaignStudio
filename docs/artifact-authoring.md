...
For YouTube, provide only the exact 11-character video ID and an optional accessible title of up to 120 characters. Campaign Studio validates the ID at finalization. Runtime v4 initializes a sandboxed privacy-enhanced `www.youtube-nocookie.com` player directly in the placeholder when the page loads, with native controls, autoplay disabled, and inline playback enabled. This matches the section renderer's YouTube grid: the visitor clicks YouTube's own play control once, with no separate Campaign Studio load button. YouTube resources load before visitor activation. On the first confirmed playback, the runtime records a `cta_click` event with type `video`, key `video-<youtubeId>`, section `videos`, and the title (or video ID) as its label. Merely loading the player does not count as playback. Do not author YouTube URLs, iframes, player parameters, or API code.
...
Place the YouTube placeholder directly in normal document flow inside a responsive 16:9 area. Do not wrap it in a modal, lightbox, overlay, or a separate thumbnail link. Do not add `data-cs-action="cta"` to the widget or its ancestors; the runtime records confirmed plays automatically.

Existing finalized artifacts retain their pinned runtime. To adopt v4, upload and finalize a new version, review its preview, then publish it at the existing slug. Keep the previous version for rollback. Runtime v3 already supports inline placement, but requires a separate player click and does not explicitly enable inline playback on iOS.
